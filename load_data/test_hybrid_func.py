...
def main():
    ...
    #soft_tfidf = py_stringmatching.SoftTfIdf([['hej', 'då'], ['hej', 'hejsan'],  ['hej']], sim_func=Levenshtein().get_sim_score, threshold=0.8)
    threshold = 0.8
    #soft_tfidf = py_stringmatching.SoftTfIdf(corpus_list, sim_func=levenshtein_similarity, threshold=threshold)
    soft_tfidf = py_stringmatching.SoftTfIdf(corpus_list, sim_func=jaro_winkler_similarity, threshold=threshold)
    tfidf = py_stringmatching.TfIdf(corpus_list, dampen=True)
    
    tf_x, tf_y = collections.Counter(["armandos", "pizzeria"]), collections.Counter(["armandos"])
    print("tf_x ", tf_x)
    print("tf_y ", tf_y)
    ...
    for term_x in tf_x:
        ...
        for term_y in tf_y:
            ...
            score = jaro_winkler_similarity(term_x, term_y)
    ...
    #print(soft_tfidf.get_raw_score(["armandos", "pizzeria"], ["armandos"]))
    print(tfidf.get_sim_score(["armandos", "pizzeria"], ["armandos", "pizza"]))
